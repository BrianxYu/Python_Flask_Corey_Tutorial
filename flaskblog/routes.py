...
@app.route("/logout")
def logout():
    logout_user() # user is not required since it is to log out the current user
    return redirect(url_for('home'))
...
def save_picture(form_picture, output_size: tuple = (125, 125)) -> str:
    ...
    # Store to desired path
    i.save(picture_path)
    return picture_fn

@app.route("/account", methods = ['GET', 'POST'])
@login_required # this decorator will enforce the user to login before can access
def account():
...
def user_posts(username: str):
    ...
    return render_template('uesr_posts.html', posts = posts, user = user)
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Email has been sent with instruction to set your password.', 'info')
        return redirect(url_for('login'))
